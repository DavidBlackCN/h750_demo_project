...
仅 SDS2502X Plus `5.0.0.1.6.2R5` 已验证 `scope_screenshot`。该工具只读保存当前面板 PNG，不改变 RUN/STOP、不操作信号源，并在截图后复核身份。截图用于形状、触发和面板诊断；幅度/频率验收仍使用 `scope_measure_simple`。旧固件及其他型号先阅读 `device-support.md`，不可假定支持。
