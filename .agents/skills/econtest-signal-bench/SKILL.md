...
description: 通过 econtest-bench-mcp 的 USB/VISA 工具安全控制 SIGLENT SDS2000X Plus 示波器和 SDG6000X/SDG6000X-E 信号发生器。用于电赛低压信号题中的设备发现、基本波产生、示波器原生验收测量、扫频、波形证据和完整回路验证。
---
...
- 幅度、Vpp 和频率的现场验收与校准，优先使用 `scope_measure_simple` 的 `measurement_source: scope_native_simple`；使用 `PKPK`、`FREQ`、`RMS` 等与示波器前面板一致的原生项目。
- `scope_capture_waveform` 的 `measurement_role` 是 `evidence_waveform`。导出的 BYTE 波形和 `waveform.vpp_v` 仅可用于波形形状、削顶、偏置、触发和辅助算法诊断，不能取代原生 `PKPK` 作 1% 级幅度验收。
- 原生测量不可用时，才可使用波形摘要作为降级证据；报告必须明确写为“估计值”，说明导出位宽/每码伏特数和量化误差，不能声称满足精密指标。
- 报告同时列出原生测量值和波形摘要值。两者 Vpp 差异超过任务预设门限时，结果为 `inconclusive`，停止自动校准并要求人工判断。
- 单次 `scope_measure_simple` 可用 `samples` 多次读取；以返回的中位数、最小、最大和标准差作稳健判断，但保留每一次原始 `PKPK` 及 `raw_response`。
- 批量任务（如多频点、多档位）先选 1 至 3 个代表点比较原生值与导出波形摘要，达到预设一致性门限后再运行整批。禁止仅凭单一软件拟合值宣称满足 1% 指标。

## 外部受控 DUT

当 DUT 由串口、调试器、MCU 或其他非 SDG 控制器驱动时，不使用 `bench_capture_response` 开关信号源。调用方必须：

1. 自行发送 DUT 命令并验证可解析回显；从回显完成时刻开始计时。
2. 显式指定稳定时间；不得把通用短等待用于低频、高 Q 或长时间常数对象。
3. 必要时以 `scope_control(SINGLE)` 采集新帧。只有 `single.completed=true` 才继续；Ready、超时和原生无效值都是 `inconclusive`。
4. 调用 `scope_measure_simple`，将同一 `measurement_point_id` 与 `dut_context`（命令、回显及时间戳）写入证据。
5. 调用 `scope_capture_waveform` 时使用相同测点 ID 保存证据，且不以其 Vpp 更新标定表。
6. 最终将示波器 STOP 并回读确认。外部 DUT 的最终输出状态由调用方记录，绝不称为 MCP 已关闭。
...
每个 manifest 均有 `calibration_eligible`。当前独立工具默认是 `false`；没有原生测量、完整配置回读、DUT 回显和已验证的新鲜采集这四项时，禁止把任何结果标作可用于校准。
