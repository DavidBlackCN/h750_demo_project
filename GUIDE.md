...
当前主任务使用 ADS8688 自动扫描内部通道 `0`～`3`，对应模块标注的 `CH1`～`CH4`。四路均配置为 `+-10.24 V` 量程，目标采样率为每通道 `100 kS/s`（总转换率 `400 kS/s`）。
...
3. 当前 demo 每帧仅发送 CH1，格式为 `CH1:<voltage>\r\n`，例如 `CH1:1.234567\r\n`。不要在同一串口中混入其他文本日志。
4. UART 通过 DMA 异步发送；串口忙时只保留最新一帧，因此不会阻塞采集。程序每采集 25 组四路样本发送一行，CH1 显示率为 4 kframes/s。
...
5. 若 VOFA+ 无数据，先检查 `PB12/PB13/PB15` 是否有 CS、SCK 和 SDI 时序，再检查 `PB14` 是否返回数据；初始化会读回自动扫描寄存器，读回不匹配时主程序进入 `Error_Handler()`。
