...
- USART1 使用 `PB6/PB7`、115200 8N1。上电完成 USART1 初始化后先输出 `ok`；采样和 FFT 完成后只输出 `begin count`、`fa_j`、`Upp`、`Urms`、各 `n=...` 分量和 `end` 最终测量摘要，不发送原始波形或半边频谱；`fa_j`、`f_Hz`、`fit_Hz` 均按整数 Hz 输出，`Upp`/`Urms` 单位为 mV，`phase_deg` 单位为度。码值换算后，帧先经过 63 阶零相位 FIR（500 kHz 约 0.001 dB、1 MHz 约 −90.7 dB）再作 FFT 与拟合，以抑制第 3 项的 `uJ`。FFT 谱峰搜索范围为 10～505 kHz，为题目第 2 项的 500 kHz 边界留出频点余量。谐波最高次数不再固定为 4，而是由搜索范围动态推导；当前 10 kHz 基频可识别至 50 次。基频候选仍要求 0.5 mVpeak 的原始帧相关确认，避免噪声子谐波误判；随后对 DC 和已识别谐波作联合正弦最小二乘拟合，输出经拟合更新的谐波 Vpp/相位。ADC 输入标定当前为 3.3 V 基准、前端增益 1，尚需实物校准。高于 ADC 奈奎斯特频率的干扰仍须由模拟前端先抑制。
