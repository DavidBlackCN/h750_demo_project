...
- AD9910 当前运行波形 API demo，输出端不接硬件放大。正弦波、RAM 方波和 RAM 三角波均已由用户确认正常；三角波原50点回放存在可见阶梯，现改为在50～1024点间搜索频率误差最小且点数最多的 RAM 时序。100 kHz 选择625点、地址步进率4，理论频率保持100 kHz，等待上板确认阶梯改善效果。RAM 配置保持先禁用、装载后再使能，CFR1[16] 选择正弦输出，以90°/270°相位映射正负样本。
