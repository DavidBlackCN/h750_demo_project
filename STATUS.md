...
- 后续开发请避免回退这些未确认来源的文件。
## 2026-07-22 要求 4 串口自动校准

- USART1 已实测使用 CH340 `COM6`、921600 8N1；`Tools/serial_tool.py` 可发送 `r4 <Hz> <Vpp>` 并确认板端 `RX` 与 `OUT R4` 回显。
- `BLL/G_MODEL_BLL.c` 的 30 点已根据 CH1（十倍放大器和已知模型之后）在 100 Hz 至 3 kHz、每点 1.0 至 2.0 Vpp 共 330 个实测数据完成更新；Debug 构建、OpenOCD 烧录与校验均通过。
- 更新后第二轮 330 点复测的平均绝对误差为 0.228%，中位增益比均接近 1。个别超过 1% 的 7 个结果均为 1.8 Vpp 档的同向 20 mV 量化偏差；相同频点其余 10 档最大绝对误差为 0.41%，不应以频点增益表补偿该测量量化误差。
- 本轮采集脚本为 `Tools/calibrate_requirement4.py`，原始示波器证据及两轮汇总位于 `artifacts/`。
